  ...
  kubectl port-forward --namespace default svc/rapids-release-scheduler :8786 &
  kubectl port-forward --namespace default svc/rapids-release-scheduler :80 &

  export JUPYTER_NOTEBOOK_IP=127.0.0.1
  export JUPYTER_NOTEBOOK_PORT=8082
  kubectl port-forward --namespace default svc/rapids-release-jupyter :80 &
